...
public class TestIndex {
	...
	public void testIndex() {
		...
		jsonObject.addProperty("date", "2018-10-7");
	...
	public void testUpdate() {
		...
		jsonObject.addProperty("name", "java编程思想");
		jsonObject.addProperty("date", "2018-10-7");
		jsonObject.addProperty("price", "200");
	...
	public void testIndex1()throws Exception{
	    ...
	    for(int i=0;i<jsonArray.size();i++){
	        ...
	        IndexResponse response=client.prepareIndex("film", "xiju")
	...
	public void searchPaging() {
		...
		SearchResponse searchResponse = builder.setQuery(QueryBuilders.matchAllQuery())
			.setFrom(1)
			.setSize(3).get();
	 ...
	 */
	@Test
	public void includeFied() {
		SearchRequestBuilder builder = client.prepareSearch("film").setTypes("dongzuo");
		SearchResponse searchResponse = builder.setQuery(QueryBuilders.matchAllQuery())
			.setFetchSource(new String[] {"title","publishDate","price"}, null)
			.setFrom(1)
			.setSize(3).get();
		SearchHits hits = searchResponse.getHits();
		for (SearchHit searchHit : hits) {
	...
	public void searchByCondition() {
		SearchRequestBuilder builder = client.prepareSearch("film").setTypes("dongzuo");
		SearchResponse searchResponse = builder.setQuery(QueryBuilders.matchQuery("title", "战"))
	...
	public void searchMulti() {
		SearchRequestBuilder builder = client.prepareSearch("film").setTypes("dongzuo");
		MatchPhraseQueryBuilder matchPhraseQuery = QueryBuilders.matchPhraseQuery("title", "战");
		MatchPhraseQueryBuilder matchPhraseQuery2 = QueryBuilders.matchPhraseQuery("content", "星球");
		
		SearchResponse searchResponse = builder.setQuery(QueryBuilders.boolQuery()
				.must(matchPhraseQuery)
				.must(matchPhraseQuery2))
	...
	public void searchMulti1() {
		SearchRequestBuilder builder = client.prepareSearch("film").setTypes("dongzuo");
		MatchPhraseQueryBuilder matchPhraseQuery = QueryBuilders.matchPhraseQuery("title", "战");
		RangeQueryBuilder lte = QueryBuilders.rangeQuery("price").gte(20);
		SearchResponse searchResponse = builder.setQuery(QueryBuilders.boolQuery()
				.must(matchPhraseQuery)
				.must(lte))
				.setFetchSource(new String[] {"title","price"}, null)
